    ...
    public function render(): View
    {
        ...
        $evenings = Evening::query()
            ...
            ->when($this->periodUntil, fn (Builder $query, string $date): Builder => $query
                ->where('played_at', '<', $date))
            ->when($this->eveningTypeId, fn (Builder $query, int $id): Builder => $query
                ->where('evening_type_id', $id))
            ->when($this->projectId, fn (Builder $query, int $id): Builder => $query
                ->where('project_id', $id))
            ->orderByDesc('played_at')
            ->simplePaginate(5, pageName: 'staffEveningsPage');
